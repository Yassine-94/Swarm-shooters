...
class Master_Bot():
    def __init__(self):
        self.hostAddress = '169.254.162.114' # The MAC address of a Bluetooth adapter on the server. The server might have multiple Bluetooth adapters.
    ...
    def server_connection(self):
        ...
        s.bind((self.hostAddress,self.port))
        #s.listen(self.backlog)
        s.listen()
        connexion_client, address = s.accept()

    def receive_distance(self):
        while 1:
            data = connexion_client.recv(self.size)
    ...
    def sound(self) :
        speak = Sound()
        speak.speak('Without connection')
